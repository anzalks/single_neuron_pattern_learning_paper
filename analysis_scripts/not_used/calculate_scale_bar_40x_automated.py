...
def process_image_pillow(image_path, physical_dist_in_um, num_bars=2):
    ...
    window_length = min(51, len(projection) // 2 * 2 - 1)  # Ensure window_length is valid
...
def measuring_points(image_with_annotations, um_per_pixel, bar_length_um, point1, point2):
    """
    Overlays the scale bar on the annotated image and displays it with the points and line.
    """
...
def main():
    ...
    # Step 2: Overlay the scale bar and plot points with a line in a separate figure
    measuring_points(annotated_image, um_per_pixel, bar_length_um, point1, point2)
