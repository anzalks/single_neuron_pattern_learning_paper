...
def eq_fit(list_of_x_y_responses_pre, list_of_x_y_responses_post, label_post, cell_type, fig, axs, plot_pre_data=True):
    """
    Fits the gamma function model to pre-training and/or post-training data,
    and plots the results on the provided axes.
    """
    # Convert post-training data to numpy array
    x_y_responses_post = np.array(list_of_x_y_responses_post)
    arr1_post, arr2_post = np.split(x_y_responses_post, 2, axis=1)
    arr1_post = np.ravel(arr1_post)
    arr2_post = np.ravel(arr2_post)

    # Fit the gamma model for post data
    param_post, _ = scipy.optimize.curve_fit(gama_fit, arr1_post, arr2_post, bounds=(0, 60))
    x_post = np.linspace(-0.5, 10, len(arr2_post))
    y_post = gama_fit(x_post, param_post[0])

    # Plot the post data fit with specific colors for learners and non-learners
    color_post = bpf.CB_color_cycle[0] if cell_type == "learners" else bpf.CB_color_cycle[1]
    axs.plot(x_post, y_post, color=color_post, linestyle='-', alpha=0.8, linewidth=3, label=label_post)

    # Ensure only one line for post data fit is plotted
    axs.legend(frameon=False, loc='upper right')

    # If plot_pre_data is True, fit and plot pre-training data
    if plot_pre_data and list_of_x_y_responses_pre:
        x_y_responses_pre = np.array(list_of_x_y_responses_pre)
        arr1_pre, arr2_pre = np.split(x_y_responses_pre, 2, axis=1)
        arr1_pre = np.ravel(arr1_pre)
        arr2_pre = np.ravel(arr2_pre)

        # Fit the gamma model for pre data
        param_pre, _ = scipy.optimize.curve_fit(gama_fit, arr1_pre, arr2_pre, bounds=(0, 60))
        x_pre = np.linspace(-0.5, 10, len(arr2_pre))
        y_pre = gama_fit(x_pre, param_pre[0])

        # Plot the pre data fit using black for consistency
        axs.plot(x_pre, y_pre, color='black', linestyle='-', alpha=0.8, linewidth=3)
        axs.text(0.7, 0.75, f'γ_pre = {np.around(param_pre[0], 1)}', transform=axs.transAxes, fontsize=12, ha='center')

    return x_post, y_post



def plot_expected_vs_observed_all_trials(alltrial_Df, mean_all_cell_df, sc_data_dict, cell_type, fig, axs1, axs2, axs3, plot_pre_data=True):
    """
    Plots the expected vs observed responses for all trials, separated by learners and non-learners.
    """
    # Filter cells based on type (learners or non-learners)
    ...
    response_dict_pre = {i: [] for i in range(3)}
    response_dict_post = {i: [] for i in range(3)}
        ...
        for pp, pp_data in pp_grps:
            ...
            for trial, trial_data in trial_grp:
                ...
                for pat in pats:
                    ...
                    if pp == 'pre' and plot_pre_data:
                        response_dict_pre[pat_num].append([point_sum_val, pat_val])
                    elif pp == 'post_3':
                        response_dict_post[pat_num].append([point_sum_val, pat_val])

                    color = bpf.pre_color if pp == 'pre' else bpf.CB_color_cycle[0] if cell_type == "learners" else bpf.CB_color_cycle[1]
                    axs[pat_num].scatter(point_sum_val, pat_val, color=color, alpha=0.8, marker=".")
                    axs[pat_num].axline([0, 0], [1, 1], linestyle=':', color='gray', linewidth=2)
                    axs[pat_num].spines[['right', 'top']].set_visible(False)

    # Fit and plot the gamma model for learners and non-learners
    for pat_num in range(3):
        pre_responses = response_dict_pre[pat_num] if plot_pre_data else None
        post_responses = response_dict_post[pat_num]

        if post_responses:
            label_post = "γ_post_learner" if cell_type == "learners" else "γ_post_non_learner"
            eq_fit(pre_responses, post_responses, label_post, cell_type, fig, axs[pat_num], plot_pre_data)
...
def plot_figure_7(extracted_feature_pickle_file_path,
    ...
    gs = GridSpec(14, 8, width_ratios=width_ratios, height_ratios=height_ratios, figure=fig)
    ...
    # Plot summation for learners
    axs_ex_sm1 = fig.add_subplot(gs[4:6, 0:2])
    axs_ex_sm2 = fig.add_subplot(gs[4:6, 2:4])
    axs_ex_sm3 = fig.add_subplot(gs[4:6, 4:6])
    plot_expected_vs_observed_all_trials(
        alltrial_Df, feature_extracted_data, sc_data_dict, "learners",
    ...
    # --- New Row for Combined Post-Training Learners (Blue) and Non-Learners (Orange) ---
    # Add 3 more axes for the new row
    axs_comb1 = fig.add_subplot(gs[10:12, 0:2])
    axs_comb2 = fig.add_subplot(gs[10:12, 2:4])
    axs_comb3 = fig.add_subplot(gs[10:12, 4:6])
    
    # Use the same data but plot only post-training (post_3) for both learners and non-learners
    for i, (ax, pat_num) in enumerate(zip([axs_comb1, axs_comb2, axs_comb3], range(3))):
        # Initialize lists to store post-training responses for learners and non-learners
        post_learners_responses, post_nonlearners_responses = [], []
    
        for cell_type, color in zip(["learners", "non-learners"], [bpf.CB_color_cycle[0], bpf.CB_color_cycle[1]]):
            # Select cells based on type
            if cell_type == "learners":
                cells = sc_data_dict["ap_cells"]["cell_ID"].unique()
            else:
                cells = sc_data_dict["an_cells"]["cell_ID"].unique()
    
            filtered_df = alltrial_Df[alltrial_Df["cell_ID"].isin(cells)]
            cell_grp = filtered_df.groupby(by='cell_ID')
            ...
            for c, cell in cell_grp:
                ...
                # Only process post-training data
                if 'post_3' not in pp_grps.groups:
                    continue
                pp_data = pp_grps.get_group('post_3')
                ...
                for trial, trial_data in trial_grp:
                    ppresp = pp_data[pp_data["trial_no"] == trial].reset_index(drop=True)
                    for pat in pats:
                        pat_num_inner = int(pat.split("_")[-1])
                        if pat_num_inner != pat_num:
                            continue
                        ...
                        # Append data to the respective list based on cell type
                        if cell_type == "learners":
                            post_learners_responses.append([point_sum_val, pat_val])
                        else:
                            post_nonlearners_responses.append([point_sum_val, pat_val])
    
                        # Plot scatter points for learners and non-learners
                        ax.axline([0, 0], [1, 1], linestyle=':', color=bpf.CB_color_cycle[6], linewidth=2)
                        ax.scatter(point_sum_val, pat_val, color=color, alpha=0.8, linewidth=1, marker=".")
    
                        ax.spines[['right', 'top']].set_visible(False)
                        ax.set_xlim(-0.5, 12)
                        ax.set_ylim(-0.5, 12)
                        ax.set_xticks(np.arange(-0.5, 12, 4))
                        ax.set_yticks(np.arange(-0.5, 12, 4))
        ...
        # Fit and plot the gamma model for learners (blue) and non-learners (orange)
        eq_fit(post_learners_responses, post_learners_responses, pat_num, "learners", fig, ax)
        eq_fit(post_nonlearners_responses, post_nonlearners_responses, pat_num, "non-learners", fig, ax)
    ...
    plt.tight_layout()
    outpath = f"{outdir}/figure_7_extended.png"
...
#
#    plt.tight_layout()
#    outpath = f"{outdir}/figure_7_with_comparison.png"
