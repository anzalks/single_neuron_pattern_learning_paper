...
cell_stats =pd.read_hdf("/Users/anzalks/Documents/pattern_learning_paper/data/CA1_3_pattern_training_experiment_all_cells/cell_health_stats.h5")
...
def fist_element(np_array):
    if isinstance(np_array, np.ndarray):
        if np.shape(np_array)[0]>1:
            if np_array[0]>1:
                np_array = np_array[0]
            else:
                np_array = np_array[0]
            ...
            np_array =np_array
    elif isinstance(np_array, tuple):
        if len(np_array)>1:
            np_array = np_array[0]
        else:
            np_array =np_array
...
def first_element(np_array):
    if isinstance(np_array, np.ndarray):
        if np.shape(np_array)[0]>1:
            ...
            first_element =np_array

    elif isinstance(np_array, tuple):
        if len(np_array)>1:
            first_element = float(np_array[0])
        else:
            first_element =float(np_array)
...
for a in cell_stats.iterrows():
    if a[1]['cell_stats']['cell_status']=='valid':
        h_cells.append(a[0])
        h_cell_stats.append(a)
cn = np.arange(0,len(h_cells),1)
dict_hc = dict(zip(h_cells,cn))

all_data_with_training_df = read_pkl("all_data_with_training_df.pickle")

all_data_with_training_df = all_data_with_training_df[all_data_with_training_df["cell_ID"]!="2022_12_12_cell_5"]


#all_cell_trace_alone = all_cell_trace_alone[all_cell_trace_alone["cell_ID"]!="2022_12_12_cell_5"]
def extract_cell_features_all_trials(cells_df):
                ...
                for pat, patg in pat_grp:
                    if ("no_frame" in pat)or("inR" in pat):
                        ...
                        for tr, trial in trial_grp:
                            ...
                            cell_list.append([c,fs,pps,pat,tr,min_trace,max_trace,abs_area,pos_area,neg_area,onset_time,max_field, min_field,slope,intercept,min_trace_t,max_trace_t,max_field_t, min_field_t,pat_trace,field_trace,ttl_trace, cell_rmp])
    clist_header=["cell_ID","frame_status","pre_post_status","frame_id","trial_no","min_trace","max_trace","abs_area","pos_area","neg_area","onset_time","max_field","min_field","slope","intercept","min_trace_t","max_trace_t","max_field_t","min_field_t","trace","field","ttl","mean_rmp"]
    ...
    return pd_cell_list
#pd_all_cells = compare_spread_field_cell(all_cell_trace_alone)

pd_all_cells_all_trials = extract_cell_features_all_trials(all_data_with_training_df)

write_pkl(pd_all_cells_all_trials,"pd_all_cells_all_trials")

#cell_grp = all_cell_trace_alone.groupby(by="cell_ID")
cell_grp = all_data_with_training_df.groupby(by="cell_ID")
#cell_grp=all_cell_trace_alone_1.groupby(by="cell_ID")
#cell_grp=trail_cells.groupby(by="cell_ID")
cell_type_list = []
for c, cell in tqdm(cell_grp):
    if c not in h_cells:
        continue
            ...
            for pps, pp_status in pp_status_grp:
                ...
                for pat, patg in pat_grp:
                    if ("no_frame" in pat)or("inR" in pat):
                        ...
                        cell_type_list.append([c,fs,pps,pat,min_trace,max_trace,abs_area,pos_area,neg_area,onset_time,max_field, min_field,slope,intercept,min_trace_t,max_trace_t,max_field_t, min_field_t,mean_trace,mean_field,mean_ttl, cell_rmp])
ctype_header=["cell_ID","frame_status","pre_post_status","frame_id","min_trace","max_trace","abs_area","pos_area","neg_area","onset_time","max_field","min_field","slope","intercept","min_trace_t","max_trace_t","max_field_t","min_field_t","mean_trace","mean_field","mean_ttl","mean_rmp"]
...
def extract_cell_inR_features(cells_df):
    cell_grp = cells_df.groupby(by="cell_ID")
    cell_list_inR = []
    for c, cell in tqdm(cell_grp):
                    ...
                    for fr_id, fr_id_data in frame_id_grp:
                        if "inR" in fr_id:
                            ...
                            for trial_no, trial_data in trial_grp_data:
                                ...
                                cell_list_inR.append([c,pp_type,fr_status,trial_no,sag,inR,trace])
                        else:
                            continue        
    cell_ir_list_header = ["cell_ID","pre_post_status","frame_status","trial_number","sag","inR","trace"]    
...
pd_cell_list_inR_all_trials = extract_cell_inR_features(all_data_with_training_df)

write_pkl(pd_cell_list_inR_all_trials,"pd_cell_list_inR_all_trials")
...
all_cells_classified = {"ap_cells": ap_cells_df,"an_cells":an_cells_df}#,"ssp_cells":ssp_cells_df,"ssn_cells":ssn_cells_df}
...
#ssp_cells_df = pd.concat(ssp_cells)
#ssn_cells_df = pd.concat(ssn_cells)
all_cells_classified_fnorm = {"ap_cells": ap_cells_df_fnorm,"an_cells":an_cells_df_fnorm}#,"ssp_cells":ssp_cells_df,"ssn_cells":ssn_cells_df}

write_pkl(all_cells_classified_fnorm,"all_cells_classified_fnorm")